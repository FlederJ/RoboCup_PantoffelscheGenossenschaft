...
Adafruit_TCS34725_SW colour2 = Adafruit_TCS34725_SW(TCS34725_INTEGRATIONTIME_50MS, TCS34725_GAIN_4X);


ZumoMotors motors ;
ZumoReflectanceSensorArray reflektionsSensoren;

uint16_t r; uint16_t r2; uint16_t g; uint16_t g2; uint16_t b; uint16_t b2; uint16_t h; uint16_t h2;
bool wl; bool wr; bool bl; bool br; bool gl; bool gr;
...
void printArray(bool Array[]) {
  for (int i = 0; i < 5; i++) {
    Serial.print("Array Part ");Serial.print(i+1);Serial.print(":");Serial.print(Array[i]);Serial.print("\n");
  }
}
...
void driving(bool bl, bool wl, bool gl,bool br, bool wr, bool gr) { /* Die b und w sind noch von colour, aber später mit reflectanceArray machen und daraus die Stärke des "Schwankens" berechnen*/
  ...
  bool g = gr || gl;
  if (wl && wr) {
    Serial.print("drive forward");Serial.print("\n");
    motors.setSpeeds(200,200);
  } else if (bl && wr) {
    Serial.print("to left");Serial.print("\n");
    motors.setRightSpeed(300);
  } else if (br && wl) {
    Serial.print("to right");Serial.print("\n");
...
bool DoGreen(int LRB) {
  uint16_t r; uint16_t g; uint16_t b; uint16_t h; int count;
  if (LRB == 0) {
...
void resetValues() {
  bool br = false; bool bl = false; bool wr = false; bool wl = false; bool gr = false; bool gl = false;
}
